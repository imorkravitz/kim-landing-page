...
const MOCK_STORIES = [
  {
    ...
    weight_lost: 12,
    duration_months: 4,
    quote: "השינוי הכי גדול היה באנרגיות. ירדתי 12 קילו אבל הרווחתי את החיים שלי בחזרה.",
    image: img1,
    is_featured: true
  },
  {
    ...
    weight_lost: 8,
    duration_months: 3,
    quote: "מעולם לא חשבתי שאוכל ליהנות מאוכל ועדיין לרדת במשקל. השיטה של ה-80/20 פשוט עובדת.",
    image: img2,
    is_featured: true
  },
  {
    ...
    weight_lost: 15,
    duration_months: 6,
    quote: "אחרי הלידות לא הצלחתי לחזור לעצמי. הליווי האישי נתן לי את הביטחון והכלים.",
    image: img3,
    is_featured: false
  },
];
...
export default function SuccessStories() {
  ...
  const avgDuration = stories.length > 0 
    ? Math.round(stories.reduce((sum, s) => sum + (s.duration_months || 0), 0) / stories.length)
    : 0;
      ...
      {showForm && (
        <section className="py-12 px-6 bg-white border-y border-gray-100">
          <div className="max-w-3xl mx-auto">
            ...
            <SubmitStoryForm onSuccess={() => setShowForm(false)} />
          </div>
        </section>
      )}
